...
class UploadComponent:
    ...
    def render_uploaded_files(self) -> None:
        ...
        for idx, file_info in enumerate(st.session_state.uploaded_files):
            expander_key = f"expander_{file_info['saved_filename']}"
            with st.expander(f"{file_info['original_filename']} ({file_info['file_type'].upper()})", key=expander_key):
                ...
                if extracted_text:
                    ...
                    st.text_area("Full Content", value=extracted_text, height=200, key=f"full_text_{idx}")
                    ...
                    if not tts_component:
                        ...
                        for seg_idx, segment_text in enumerate(segments):
                            ...
                            segment_id = f"{file_info['saved_filename']}_seg_{seg_idx}"
                            ...
                            with button_col:
                                if st.button(f"🔊 Play", key=f"play_{segment_id}", help="Read this paragraph aloud"):
                ...
                if st.button(f"Remove File", key=f"remove_{idx}"):
        ...
        if files_to_remove_indices:
            ...
            for index in sorted(files_to_remove_indices, reverse=True):
                ...
                try:
                    if removed_file.get('file_path') and os.path.exists(removed_file['file_path']):
